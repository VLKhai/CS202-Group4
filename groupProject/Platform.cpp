...
void Platform::draw(sf::RenderWindow& mainWindow) {
	if (iType == 6 || iType == 7) { // -- iXStart = YPos 2
		for (int i = 1; i < (iXStart - iYEnd) / 16 + 2; i++) {
			Core::getMap()->getBlock(Core::getMap()->getCurrentLevelID() == 22 ? 165 : 135)->getAniSprite()->getFrame()->draw(mainWindow, (int)(iXEnd + (int)Core::getMap()->getXPos() + (iSize * 8) / 2 + 16 - (iSize / 2 % 2 == 0 ? 8 : 0)), (int)(iXStart - 16 * i));
		}
	}

	for (int i = 0; i < iSize; i++) {
		Core::getMap()->getBlock(iType != 4 ? 74 : 126)->draw(mainWindow, (int)fXPos + i * 16 + (int)Core::getMap()->getXPos(), (int)fYPos);
	}
}
...
void Platform::moveY() {
	...
	case 6:
		fYPos += 2;
		iXStart = (int)fYPos;
		Core::getMap()->getPlatform(seesawPlatformID)->moveYReverse();
		if (fYPos < iYEnd) {
			iType = 7;
			Core::getMap()->getPlatform(seesawPlatformID)->setTypeID(7);
		}
		break;
